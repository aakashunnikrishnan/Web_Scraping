...
def fetch_page(slug, cursor=None, limit=20):
    payload = {
...
def fetch_all_answers(slug, cursor=None, collected=None):
    if collected is None:
        collected = []

    answers, next_cursor, has_next = fetch_page(slug, cursor)
    ...
    if has_next and next_cursor:
        return fetch_all_answers(slug, next_cursor, collected)
...
if __name__ == "__main__":
    ...
    all_answers = fetch_all_answers(slug)
